...
const pintarCarrito = () => {
  ...
  carrito.forEach((i) => {
    html += `<article class="cart-card">
            ...
            <h4 class="cart-price">$<span>${new Intl.NumberFormat(
              "es-MX"
            ).format(i.precio * i.cantidad)}</span></h4>
            </div>
            <div class="cart__btns">
            <input type="button" value="Agregar" class="cart-card__btn card-add" data-id="${
              i.id
            }"><input type="button" value="Quitar" class="cart-card__btn card-clear" data-id="${
      i.id
    }">
            </div></article>`;
  });
...
const pintarCantidad = () => {
  let contador = carrito.reduce((acc, current) => acc + current.cantidad, 0);
